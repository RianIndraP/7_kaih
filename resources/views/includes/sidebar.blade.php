...
<nav class="flex-1 flex flex-col gap-0.5 px-2.5 py-3.5 overflow-y-auto sb-scroll">
    ...
    {{-- Dashboard --}}
    <a href="{{ route('student.dashboard') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
    ...
    {{-- Profil Siswa --}}
    <a href="{{ route('student.profile') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
              no-underline transition-all duration-200
    ...
    @if (auth()->user()->isProfileComplete())
        <a href="{{ route('student.kebiasaan') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
                              no-underline transition-all duration-200
    ...
    @if (auth()->user()->isProfileComplete())
        @php
            $unreadCount = \App\Models\PesanGuruSiswa::where('siswa_id', auth()->id())
                ...
                ->count();
        @endphp
        <a href="{{ route('student.pesan') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
                              no-underline transition-all duration-200
            ...
            @if ($unreadCount > 0)
                <span class="ml-auto bg-red-500 text-white text-[10px] font-bold
    ...
    @if (auth()->user()->isProfileComplete())
        <a href="{{ route('student.kuis') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
                              no-underline transition-all duration-200
                              {{ request()->routeIs('student.kuis*')
            ? 'bg-white/22 text-white font-bold nav-active-accent'
            : 'text-white/72 hover:bg-white/14 hover:text-white hover:translate-x-[3px]' }}">
            <span class="w-[17px] h-[17px] shrink-0 flex items-center justify-center text-lg">💡</span>
            <span class="flex-1">Kuis Literasi & Numerasi</span>
        </a>
    @else
        <div class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
                                opacity-45 cursor-not-allowed text-white/72 bg-white/5">
