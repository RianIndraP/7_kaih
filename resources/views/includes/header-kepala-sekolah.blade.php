...
<header
    ...
    <div class="flex items-center gap-2.5 min-w-0">
        ...
        <h1 class="text-[15px] font-bold text-white truncate">
            @if (request()->routeIs('kepala-sekolah.dashboard'))
                Dashboard
            @elseif(request()->routeIs('kepala-sekolah.profil'))
                Profil Kepala Sekolah
            @elseif(request()->routeIs('kepala-sekolah.data-kebiasaan'))
                Data Kebiasaan
            @elseif(request()->routeIs('kepala-sekolah.data-guru-wali'))
                Data Guru Wali
            @elseif(request()->routeIs('kepala-sekolah.data-siswa'))
                Data Kelas
            @else
                {{ $pageTitle ?? 'Dashboard' }}
            @endif
    ...
    <div class="flex items-center gap-1 shrink-0">
        ...
        {{-- Gear: Ganti Password --}}
        <a href="{{ route('kepala-sekolah.ganti-password') }}" title="Ganti Password"
        ...
        <a href="{{ route('kepala-sekolah.kirim-pesan-bantuan') }}" title="Kirim Pesan Bantuan"
            class="flex items-center justify-center w-9 h-9 rounded-[9px]
                  bg-white/15 text-white no-underline
                  hover:bg-white/26 hover:scale-[1.08] transition-all duration-200">
            <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
