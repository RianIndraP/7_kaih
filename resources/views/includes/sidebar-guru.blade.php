...
<div class="relative flex flex-col items-center gap-2.5 px-5 pt-6 pb-5
    ...
    {{-- Mobile close button --}}
    <button id="sbCloseBtn" class="lg:hidden absolute top-3.5 right-3.5 flex items-center justify-center
    ...
    {{-- Logo image --}}
    <div class="flex items-center justify-center w-[62px] h-[62px] rounded-2xl
...
<nav class="flex-1 flex flex-col gap-0.5 px-2.5 py-3.5 overflow-y-auto sb-scroll">
    ...
    {{-- Dashboard --}}
    <a href="{{ route('guru.dashboard') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
    ...
    {{-- Profil Guru --}}
    <a href="{{ route('guru.profil') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
              no-underline transition-all duration-200
    ...
    @if (!empty(auth()->user()->email) && !empty(auth()->user()->no_telepon))
        <a href="{{ route('guru.list-murid') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
                      no-underline transition-all duration-200
    ...
    @else
        <div class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
    ...
    {{-- Absensi Murid --}}
    @if (!empty(auth()->user()->email) && !empty(auth()->user()->no_telepon))
        <a href="{{ route('guru.absensi-murid') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
                      no-underline transition-all duration-200
    ...
    {{-- Pelaporan --}}
    @if (!empty(auth()->user()->email) && !empty(auth()->user()->no_telepon))
        <a href="{{ route('guru.pelaporan') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
                      no-underline transition-all duration-200
    ...
    <a href="{{ route('guru.pemantauan-kuis') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-[11px] text-[13px] font-medium
              no-underline transition-all duration-200
              {{ request()->routeIs('guru.pemantauan-kuis*')
    ? 'bg-white/22 text-white font-bold nav-active-accent'
    : 'text-white/72 hover:bg-white/14 hover:text-white hover:translate-x-[3px]' }}">
        <span class="w-[17px] h-[17px] shrink-0 flex items-center justify-center text-lg">📝</span>
        <span class="flex-1">Pemantauan Kuis Siswa</span>
    </a>

    {{-- ── LOGOUT ── --}}
    <div class="px-2.5 py-2.5 border-t border-white/10 mt-2">
        <button onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-[11px]
