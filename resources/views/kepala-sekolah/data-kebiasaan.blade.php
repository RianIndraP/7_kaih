...
@section('content')
            ...
            <div class="bg-white rounded-xl shadow-md p-5 mb-6 border border-gray-100">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <select id="filterPeriod" onchange="toggleFilterInputs()"
                        ...
                        <option value="weekly" {{ request()->get('filter') == 'weekly' ? 'selected' : '' }}>🗓️ Per Minggu
                        </option>
                        <option value="monthly" {{ request()->get('filter') == 'monthly' ? 'selected' : '' }}>📅 Per Bulan
                        </option>
                    </select>
                    ...
                    <div id="weeklyFilter" class="{{ request()->get('filter') == 'weekly' ? '' : 'hidden' }}">
                        <select id="weekNumber"
                            ...
                            @for($i = 1; $i <= 5; $i++)
                                <option value="{{ $i }}" {{ request()->get('week') == $i ? 'selected' : '' }}>Minggu {{ $i }}
                                </option>
                            @endfor
                        </select>
                    ...
                    <div id="monthlyFilter" class="{{ request()->get('filter') == 'monthly' ? '' : 'hidden' }}">
                        <input type="month" id="filterMonthly"
                            value="{{ request()->get('monthly') ?? now()->format('Y-m') }}"
                    ...
                    <div id="weekLogicInfo" class="{{ request()->get('filter') == 'weekly' ? '' : 'hidden' }}">
                        <div
                            class="bg-linear-to-r from-indigo-50 to-blue-50 border border-indigo-200 rounded-xl p-4 shadow-sm">
                            ...
                            <div class="grid grid-cols-4 gap-2 text-xs">
                                <div class="font-semibold text-indigo-600 text-center pb-1 border-b border-indigo-200">
                                    Minggu</div>
                                <div class="font-semibold text-indigo-600 text-center pb-1 border-b border-indigo-200">31
                                    Hari</div>
                                <div class="font-semibold text-indigo-600 text-center pb-1 border-b border-indigo-200">30
                                    Hari</div>
                                <div class="font-semibold text-indigo-600 text-center pb-1 border-b border-indigo-200">28
                                    Hari</div>
                ...
                <input type="hidden" id="startDate"
                    value="{{ request()->get('start_date') ?? now()->startOfWeek()->format('Y-m-d') }}">
                <input type="hidden" id="endDate"
                    value="{{ request()->get('end_date') ?? now()->endOfWeek()->format('Y-m-d') }}">
            </div>
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <div class="flex items-center">
                                                <div class="shrink-0 h-10 w-10">
                                                    ...
                                                    @if($displayUser && $displayUser->foto)
                                                        <img class="h-10 w-10 rounded-full object-cover"
                                                            ...
                                                            alt="{{ ucwords($displayUser->name ?? 'Unknown') }}">
                                                    @else
                                                        <div
                                                            ...
                                                            <span class="text-purple-600 font-medium text-sm">
                                                                {{ substr(ucwords($displayUser->name ?? 'Unknown'), 0, 1) }}
                                                            </span>
                                                        </div>
                                                    @endif
                                                </div>
                                                <div class="ml-4">
                                                    <div class="text-sm font-medium text-gray-900">
                                                        {{ ucwords($displayUser->name ?? 'Unknown') }}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <div class="flex items-center justify-center">
                                                <span
                                                    class="text-lg font-semibold {{ $data['presentase_penyelesaian'] >= 80 ? 'text-green-600' : ($data['presentase_penyelesaian'] >= 60 ? 'text-yellow-600' : 'text-red-600') }}">
                    ...
                    <div class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-600 text-sm max-w-md mx-auto">
                                Silakan pilih periode filter (Per Hari, Per Minggu, atau Per Bulan) untuk melihat data kebiasaan
                                siswa.
                            </p>
                        </div>
                    </div>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-blue-700">
                            <p>• Presentase dihitung berdasarkan total kebiasaan yang diselesaikan oleh semua siswa guru
                                wali tersebut.</p>
    ...
    <div id="detailModal"
        ...
        <div
            class="relative top-10 mx-auto p-0 w-11/12 md:w-4/5 lg:w-4/5 shadow-2xl rounded-2xl bg-white overflow-hidden transform transition-all">
