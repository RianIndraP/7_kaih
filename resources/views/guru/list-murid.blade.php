        ...
        <div id="panelKirim" class="p-5">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <div class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-800
    ...
    <div class="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6" onclick="event.stopPropagation()">
        <div class="flex items-center gap-3 mb-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-0.5">Apakah kamu yakin ingin menghapus semua umpan balik untuk siswa ini?</p>
            </div>
        </div>
...
function periodeLabel(p) {
    return { harian:'tanggal', mingguan:'minggu', pertemuan:'pertemuan', bulanan:'bulan' }[p] || 'filter';
}
...
function renderTabel(data, periode) {
    ...
    if (!filtered.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="px-4 py-10 text-center text-gray-400 text-sm">Tidak ada data siswa</td></tr>';
        return;
    }
...
function bukaDetail(siswa, periode) {
    ...
    var aspek = [
        {label:'Bangun Pagi', icon:'☀', val: siswa.detail?.bangun_pagi ?? 0},
        {label:'Beribadah',   icon:'🤲', val: siswa.detail?.beribadah   ?? 0},
        {label:'Berolahraga', icon:'🏃', val: siswa.detail?.berolahraga ?? 0},
        {label:'Makan Sehat', icon:'🍽', val: siswa.detail?.makan_sehat ?? 0},
        {label:'Gemar Belajar', icon:'📖', val: siswa.detail?.gemar_belajar ?? 0},
        {label:'Bermasyarakat', icon:'👥', val: siswa.detail?.bermasyarakat ?? 0},
        {label:'Tidur Cepat', icon:'🌙', val: siswa.detail?.tidur_cepat ?? 0},
    ];
...
function bukaPesan(siswa, periode) {
    ...
    document.getElementById('pesanTanggal').textContent   =
        ...
        'Juli','Agustus','September','Oktober','November','Desember'][now.getMonth()] + ' ' + now.getFullYear();
...
function switchTabPesan(tab) {
    ...
    document.getElementById('tabKirim').className  = 'flex-1 py-3 text-sm font-medium border-b-2 transition-colors ' +
        (isKirim ? 'border-blue-600 text-blue-700 bg-blue-50' : 'border-transparent text-gray-500 hover:text-gray-700');
    document.getElementById('tabHistory').className = 'flex-1 py-3 text-sm font-medium border-b-2 transition-colors ' +
        (!isKirim ? 'border-blue-600 text-blue-700 bg-blue-50' : 'border-transparent text-gray-500 hover:text-gray-700');
...
function kirimPesan() {
    ...
    if (!isi) { tampilkanToast('Isi pesan tidak boleh kosong', 'red'); return; }

    fetch('/guru/list-murid/kirim-pesan', {
        method : 'POST',
        headers: { 'Content-Type':'application/json', 'Accept':'application/json', 'X-CSRF-TOKEN':'{{ csrf_token() }}' },
        body   : JSON.stringify({ siswa_id: siswaId, judul: judul, isi: isi, periode: currentPeriode, filter: currentFilter }),
    })
    .then(function (r) { return r.json(); })
    .then(function (res) {
        if (res.success) {
...
function downloadPDF() {
    ...
    if (!rows.length) { tampilkanToast('Tidak ada data untuk didownload', 'red'); return; }
    ...
    doc.autoTable({
        startY: 28,
        head: [['No','Nama','Kelas','NISN','Penyelesaian Form','Umpan Balik']],
        body: tableData,
